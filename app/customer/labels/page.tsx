...
export default function LabelsPage() {
  ...
  const handlePrintAll = () => {
    ...
    labels.forEach((row, idx) => {
      ...
      doc.setFontSize(28);
      doc.text(
        `${idx + 1} / ${getLabelValue(row, "Parts") || labels.length}`,
        90,
        220
      );
      doc.setFontSize(12);
      ...
      doc.text(`${getLabelValue(row, "Product Description")}`, 200, 300, {
        maxWidth: 300,
      });
      doc.setFontSize(32);
      doc.text(`${getLabelValue(row, "Customer Name")}`, 400, 100, {
        maxWidth: 180,
      });
      doc.setFontSize(24);
        ...
        <div className="max-w-4xl mx-auto px-4 py-8">
          <div className="bg-white rounded-xl shadow p-6 mb-6">
            <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 flex flex-col items-center justify-center mb-4">
              ...
              <div className="text-xs text-gray-500 mt-2">
                Required columns: Order No, Delivery Date, Vehicle, Customer
                Name, Address, Phone, Product Code, Product Description,
                Quantity
              </div>
              {fileName && (
                <div className="mt-2 text-sm">Selected file: {fileName}</div>
              )}
            </div>
            <Button
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {labels.map((row, idx) => (
              <div
                key={idx}
                className="border rounded p-4 bg-white shadow text-xs"
              >
                <div>
                  <b>Order No:</b> {getLabelValue(row, "Order No")}
                </div>
                <div>
                  <b>Delivery Date:</b> {getLabelValue(row, "Delivery Date")}
                </div>
                <div>
                ...
                <div style={{ marginLeft: 16 }}>
                  {getLabelValue(row, "Address")}
                </div>
                <div>
                  <b>Assemble:</b> {row["Assemble"] || "No"}
                </div>
                <div>
                  <b>Parts:</b> {idx + 1} /{" "}
                  {getLabelValue(row, "Parts") || labels.length}
                </div>
                <div
                  style={{ fontSize: 24, fontWeight: "bold", margin: "12px 0" }}
                >
                  {getLabelValue(row, "Customer Name")}
                </div>
                <div>
                  <b>Order Details:</b>
                </div>
                <div style={{ marginLeft: 16 }}>
                  Qty: {getLabelValue(row, "Quantity")} | Product Code:{" "}
                  {getLabelValue(row, "Product Code")} | Product Description:{" "}
                  {getLabelValue(row, "Product Description")}
                </div>
              </div>
            ))}
          </div>
