  ...
  const fetchReportData = async () => {
    try {
      ...
      const productsStats = {
        ...
        addedToday:
          products?.filter((p) => new Date(p.created_at) >= today).length || 0,
        addedThisWeek:
          products?.filter((p) => new Date(p.created_at) >= thisWeek).length ||
          0,
        addedThisMonth:
          products?.filter((p) => new Date(p.created_at) >= thisMonth).length ||
          0,
      };
      ...
      const warehouseStats = {
        ...
        assigned:
          warehouseAssignments?.filter((w) => w.status === "assigned").length ||
          0,
        pending:
          warehouseAssignments?.filter((w) => w.status === "pending").length ||
          0,
      };
