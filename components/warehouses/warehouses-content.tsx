  ...
  const fetchWarehouses = async () => {
    try {
      ...
      const { data: assignmentsData, error: assignmentsError } =
        await supabase.from("warehouse_assignments").select(`
          *,
          products (
            id,
            name
    ...
    try {
      ...
      for (const assignment of newAssignments) {
        ...
        if (!currentProduct || currentProduct.quantity < assignment.quantity) {
          alert(
            `Not enough quantity available for product ID ${assignment.product_id}`
          );
          continue;
        }
        ...
        const { data: currentWarehouse, error: warehouseFetchError } =
          await supabase
            .from("warehouses")
        ...
        const newProducts =
          (currentWarehouse?.products || 0) + assignment.quantity;
  ...
  const handleUpdateAssignment = () => {
    ...
    if (assignment) {
      ...
      if (product && warehouse) {
        ...
        logStockMovement(
          "Update",
          `Updated ${quantities[assignment.productId]} of ${product.name} in ${
            warehouse.name
          }`
        );
      }
    ...
    setAssignmentSummary({
      ...
      assigned: [
        {
          productId: editingAssignmentId!.split("-")[0],
          quantity: quantities[editingAssignmentId!.split("-")[0]],
          warehouseName:
  ...
  const handleDeleteAssignment = (assignmentId: string) => {
    ...
    if (assignment) {
      ...
      if (product && warehouse) {
        ...
        logStockMovement(
          "Delete",
          `Deleted ${assignment.quantity} of ${product.name} from ${warehouse.name}`
        );
      }
      ...
      setAssignmentSummary({
        ...
        // @ts-expect-error kjhkjnj
        remaining: [
          { productId: assignment.productId, quantity: assignment.quantity },
        ],
      });
    }
  };
        ...
        <ul style="list-style: none; padding: 0;">
          ${assignedProducts
            ...
            <li style="margin-bottom: 10px; padding: 10px; border-bottom: 1px solid #ddd;">
              ...
              <strong style="color: #333;">Barcode:</strong> <img src="${generateBarcode(
  ...
  const handlePrintLabel = () => {
    ...
    if (labelElement) {
      ...
      if (printWindow) {
        ...
        alert(
          "Failed to open print window. Please allow pop-ups and try again."
        );
      }
    } else {
  ...
  const handleDeleteWarehouse = async () => {
    ...
    try {
      ...
      if (assignments && assignments.length > 0) {
        alert(
          "Cannot delete warehouse with assigned stocks. Please remove assignments first."
        );
        return;
      }
          ...
          <TableBody>
            {stockMovements.map((movement) => (
              ...
              <TableRow key={movement.id}>
                ...
                {/* @ts-expect-error kjhkjnj */}
                <TableCell>
                  {movement.warehouses?.name || "Unknown Warehouse"}
                </TableCell>
                {/* @ts-expect-error kjhkjnj */}
                <TableCell>
                ...
                <TableCell>
                  ...
                  <Badge
                    variant={
                      movement.movement_type === "assignment"
                        ? "default"
                        : "secondary"
                    }
                  >
    ...
    <div className="space-y-6">
          ...
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            ...
            <DialogContent className="max-w-2xl z-50">
              <DialogHeader>
                <DialogTitle>
                  {editingAssignmentId
                    ? "Edit Stock Assignment"
                    : "Assign Stocks to Warehouse"}
                </DialogTitle>
              </DialogHeader>
      ...
      {assignmentSummary && (
        <div className="p-4 bg-blue-100 text-blue-800 rounded-lg">
              ...
              assignmentSummary.assigned.map((item) => {
                ...
                return (
                  <li key={item.productId}>
                    {/* @ts-expect-error jkk jk */}
                    Assigned {item.quantity} of {product?.name} to{" "}
                    {item.warehouseName}.
                  </li>
                );
              })
              ...
              assignmentSummary.remaining.map((item) => {
                ...
                return (
                  <li key={item.productId}>
                    ...
                    {item.quantity} of {product?.name} could not be assigned due
          ...
          <Button onClick={handleGenerateLabel} className="mt-2">
            Generate Label
          </Button>
        </div>
      )}
      ...
      <Dialog open={isLabelDialogOpen} onOpenChange={setIsLabelDialogOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>
              Enter Address and Select Warehouse for Label
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
