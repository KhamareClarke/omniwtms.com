...
export function CouriersContent() {
  ...
  const [deletingCourierId, setDeletingCourierId] = useState<string | null>(
    null
  );
        ...
        couriers.map((c) => {
          if (c.id === selectedCourierId) {
            return {
              ...
              destination:
                deliveryType === "warehouse"
                  ? mockWarehouses.find((w) => w.id === destinationWarehouse)
                      ?.location || ""
                  : destinationAddress,
  ...
  const handleDeleteCourier = async () => {
    try {
      ...
      setCouriers(
        couriers.filter((courier) => courier.id !== deletingCourierId)
      );
  ...
  const handleRouteSelect = (route: any) => {
    ...
    toast({
      ...
      content: `New route selected with ${(
        ((route.distance * 1.2 - route.distance) / (route.distance * 1.2)) *
        100
      ).toFixed(1)}% distance reduction`,
    });
  };
                          ...
                          <div className="grid grid-cols-2 gap-x-4 gap-y-1">
                            <div className="text-sm">
                              ...
                              <span className="font-medium">
                                {log.optimizedRoute.distance.toFixed(1)} km
                              </span>
                            </div>
                            <div className="text-sm">
                              <span className="text-muted-foreground">
                              ...
                              <span className="font-medium">
                                {log.originalRoute.distance.toFixed(1)} km
                              </span>
                            </div>
                            <div className="text-sm">
                              <span className="text-muted-foreground">
                              ...
                              <span className="font-medium">
                                {(log.optimizedRoute.duration / 60).toFixed(1)}h
                              </span>
                            </div>
                            <div className="text-sm">
                              <span className="text-muted-foreground">
                              ...
                              {/* @ts-expect-error kjnjk  */}
                              <span className="font-medium">
                                {(log.originalRoute.duration / 60).toFixed(1)}h
                              </span>
                            </div>
                            <div className="text-sm col-span-2 text-green-600 font-medium">
                              ...
                              Savings: £
                              {(
                                log.originalRoute.fuelCost -
                                log.optimizedRoute.fuelCost
                              ).toFixed(2)}
                            </div>
                          </div>
                        </div>
          ...
          <TableBody>
            {couriers.map((courier) => (
              <TableRow key={courier.id}>
                ...
                <TableCell className="hidden md:table-cell">
                  {courier.destination || "N/A"}
                </TableCell>
                {/* @ts-expect-error kjh kj */}
                <TableCell className="hidden md:table-cell">
                  {formatETA(courier.eta)}
                </TableCell>
                <TableCell>
